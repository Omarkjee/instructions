...
- Current status: No public deployment was created as part of this deliverable (Azure deployment was blocked by subscription/permission constraints).
- Local URL (running on your machine or container): `http://localhost:8000`
- To provide a temporary public URL for Treasury testing, run:

```bash
# start app (Docker recommended)
docker build -t label-verifier:latest .
docker run --rm --name label-verifier -p 8000:8000 label-verifier:latest
...
If you want a long-lived public URL, ask your Azure admin to run `deploy/deploy_to_azure.sh` (requires active subscription and permission). After deployment, paste the App Service URL here and I will validate it and add it to this README.
...
Additional delivered items and how to use them:

- API key rotation: use `scripts/generate_api_key.py` to generate/list/revoke API keys. The app will read keys from `secrets/api_keys.json` when present.

- Expanded sample dataset: run `python scripts/generate_samples.py` then `python scripts/augment_samples.py` to create `static/samples_augmented` (many variants: rotated, glare, noise).

- Token guidance: to run with keys, set header `x-api-key` to a generated key.

- Expose locally: use `ngrok http 8000` to get a temporary public URL for Treasury testing.

- Audit logs & persistence: results are written to `DATA_DIR` (default `/data`) as JSONL; Dockerfile creates `/data` for persistence.

How to remove the project TODO file:

1) To delete the file from the repository working copy:

```bash
rm TODO.md
git add -A && git commit -m "Remove TODO.md after completion" && git push
```

2) To keep the file but mark complete, edit `TODO.md` and update the checkboxes.
